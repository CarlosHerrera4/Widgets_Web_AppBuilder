  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    getConfig: function () {
      ...
      if (this._validateConfigForCheckBoxes() &&
        ...
        this.config.placenameButtonText = this.placeNameTextBox.get("value");
        ...
        this.config.aoiTabText = this.aoiTabTextBox.get("value");
        this.config.reportTabText = this.reportsTabTextBox.get("value");
        this.config.drawToolsButtonText = this.drawToolTextBox.get("value");
        this.config.shapefileButtonText = this.uploadShapeFileTextBox.get("value");
        this.config.coordinatesButtonText = this.coordinatesTextBox.get("value");
        ...
        this.config.maxBufferDistance = this.maxBufferDistanceTextBox.get("value");
    ...
    _initConfiguration: function () {
      ...
      this.maxBufferDistanceTextBox.constraints = { min: 0 };
    ...
    */
    ...
    setConfig: function () {
      if (this.config) {
        ...
        if (this.config.maxBufferDistance ||
          this.config.maxBufferDistance === 0) {
          this.maxBufferDistanceTextBox.set('value', this.config.maxBufferDistance);
        }
     ...
     * @param {object} scope
     * @param {string} errorMessage
     * @param {number} changedDefaultBufferDistance
     * @memberOf Screening/setting/Setting
     */
    _validateConfigForBufferDistanceFields: function (
      scope, errorMessage, changedDefaultBufferDistance) {
      var isValid = true;
      array.some(dojoQuery(".esriCTTextBox", scope),
        lang.hitch(this, function (currentInput) {
          ...
          else if (registry.byNode(currentInput).dojoAttachPoint === "maxBufferDistanceTextBox" &&
            registry.byNode(currentInput).get("value") === 0 ||
            registry.byNode(currentInput).get("value") < changedDefaultBufferDistance) {
            this._errorMessage(this.nls.errorMsg.defaultLessThanMaxBufferDistanceFieldErrorMsg,
              this.nls.generalTab.generalTabLabel);
            isValid = false;
            return true;
          }
        }));
    ...
    _getLayerInfo: function (tr, getCapabilities) {
      ...
      if (selectedLayerItem) {
        layerItem = {
          ...
          "label": tr.labelText && tr.labelText.value ? tr.labelText.value.trim() : "",
    ...
    _createAddLayerGrid: function () {
      var args, fields = [{
        ...
        name: 'field',
        title: this.nls.analysisTab.addLayerLabelTitle,
        type: 'empty',
        editable: 'true',
        width: '200px'
      }, {
        name: 'actions',
        title: this.nls.analysisTab.addLayerActionsTitle,
        width: '80px',
        type: 'actions',
