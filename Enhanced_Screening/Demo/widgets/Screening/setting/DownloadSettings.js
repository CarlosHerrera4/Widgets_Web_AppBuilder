    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      **/
      checkLayerForDownloadOptions: function (layer, index) {
        ...
        if (layer.capabilities.split("Sync").length > 1) {
          this._addFileOption(tuple.tr, 2);
          layerInformation.downloadingFileOption.push("filegdb");
          this.loadingIndicator.show();
          // Request to check wether the layer is AGOL hosted or not
          esriRequest({
            "url": layer.url.split("/services/")[0] +
            "/info/?f=pjson"
          }).then(lang.hitch(this, function (info) {
            if (info) {
              // Show 'Shapefile' file option, if the layer is AGOL hosted
              if (info && info.owningSystemUrl &&
                info.owningSystemUrl === "http://www.arcgis.com") {
                this._addFileOption(tuple.tr, 3);
      ...
      **/
      getLogo: function () {
        ...
        if (this.logoChooser && this.logoChooser.imageData) {
          ...
          return "${appPath}/widgets/Screening/images/defaultLogo.png";
        }
      },
